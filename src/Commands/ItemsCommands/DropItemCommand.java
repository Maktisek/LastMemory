...
public class DropItemCommand implements Command {
    ...
    public String execute() {
        if (player.getInventory().getWeight() == 0){
            ...
            return Important.writeSpace(40)+Important.changeText("red", "Momentálně u sebe nemáš ani jeden předmět.");
        }
        if (!player.getCurrentLocation().addItem(player.getInventory().dropItem(name))){
            Important.playSound("wrong sound");
            return Important.writeSpace(40)+Important.changeText("red", "Momentálně u sebe nemáš " + name + ".");
        }
        Important.playSound("dropping sound");
        return Important.writeSpace(40)+"Položil si " + Important.changeText("underline", name) + " do " + player.getCurrentLocation().getName()+ ".";
    }
